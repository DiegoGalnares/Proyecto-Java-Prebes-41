...
% Last Modified by GUIDE v2.5 12-Dec-2020 02:00:47
...
% handles    structure with handles and user data (see GUIDATA)
 ...
 %LEER TABLA
 data=readtable('Copy2.csv');
 %ASIGNAR X & Y
 y=data{:,9}; %calificacion 
 x1=data{:,2}; %GRE 
 x2=data{:,3}; %TOEFL
 x3=data{:,7}; %CGPA
 %Obtener las dimensiones de Y
 [n,d]=size(y);
 n
 %Obtener los datos de x en una sola matriz
 ...
 %V
 xMat(2,2)
 %Leer los valores ingresados en la GUI
 ...
 %Transformar los datos leidos de String a Numericos
 a=str2num(gre);
 b=str2num(toefl);
 c=str2num(cgpa);
 d=str2num(pos);
